"""
run_one_config_splitk.py

Helper to execute a single configuration of the split-K CuTe DSL tensorcore GEMM
in an isolated Python process. Similar to run_one_config.py but supports split_k parameter.

It suppresses verbose prints from the underlying module and only emits a single JSON 
line to stdout with the result:
...
def main():
    p = argparse.ArgumentParser(description="Run a single CuTe GEMM config with split-K and return JSON result")
    ...
    p.add_argument("--split_k", type=int, required=True)
    ...
    import tensorop_gemm_splitk as gemm_mod  # type: ignore

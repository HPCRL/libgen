...
Runs the tunable CuTe DSL tensorcore GEMM with split-K across a sweep of tunable params
(CTA tiles, stages, atom layouts, memory layouts, split_k values) for a LIST of problem 
sizes loaded from a CSV file. Only records rows that pass correctness checking.
...
Edit tune_config_splitk.yaml to change sweep parameters.
"""
...
# We will run each configuration in an isolated subprocess via run_one_config_splitk.py
...
def _cfg_rows_equal(a: dict, b: dict) -> bool:
    """Compare two config rows on the keys that define a unique attempt (excluding timing)."""
    keys = [
        "M","N","K","L",
        "a_major","b_major","c_major",
        "cta_m","cta_n","cta_k","stages",
        "atom_m","atom_n","atom_k","split_k",
    ]
    try:
    ...
    try:
        ...
        for line in reversed(lines):
            ...
            try:
                ...
                if isinstance(cfg, dict) and {"M","N","K","L","cta_m","cta_n","cta_k","split_k"}.issubset(cfg.keys()):
                    return cfg
            except Exception:
                continue
    except Exception:
...
def _parse_last_cfg_from_csv(csv_path: Path) -> dict | None:
    """Fallback: read the last successful row from CSV and convert to cfg_row-like dict."""
...
def load_config(path: Path):
    ...
    split_k_vals = [int(s) for s in cfg.get("split_k", [1])]
...
def run_one_subprocess(cfg_row, iters, warmup, use_cold_l2, timeout_sec, skip_ref_check=False):
    """Invoke run_one_config_splitk.py with the given config and return parsed JSON or raise on protocol error."""
    args = [
        sys.executable,
        str(RUN_ONE),
        "--M", str(cfg_row["M"]),
...
def main():
    p = argparse.ArgumentParser(description="Sweep tuning params for CuTe split-K tensorop GEMM over many problems")
    ...
    p.add_argument("--timeout_sec", type=int, default=None, help="Per-config timeout in seconds (defaults to tune_config_splitk.yaml or 300)")
    # Resume options

...
int main(int argc, char** argv) {
  ...
  std::cout << "  reps=" << reps << "  warmups=" << warmups << "\n";
  ...
  // >;
  // ----- Alignment-1 TensorOp GEMM (accepts arbitrary M/N/K) -----
  using ElementInput      = cutlass::half_t;   // A,B
  using ElementOutput     = cutlass::half_t;   // C,D
  using ElementAccumulator= float;             // Accumulator

  // Epilogue: 1 element per access (so AlignmentC = 1)
  using EpilogueOp = cutlass::epilogue::thread::LinearCombination<
      ElementOutput,        // D
      1,                    // Elements per vectorized store (set to 1 to relax alignment)
      ElementAccumulator,   // Accumulator
      // ElementOutput         // C
      ElementAccumulator    // compute type for alpha/beta (float) [not sure]
  >;
  ...
  using Gemm = cutlass::gemm::device::Gemm<
      ...
      ElementAccumulator,
      cutlass::arch::OpClassTensorOp,
      cutlass::arch::Sm80,                               // works on RTX 3090 (sm_86)
      // Tile shapes
      ...
      // Epilogue
      EpilogueOp,
      // Swizzle & pipeline stages
      cutlass::gemm::threadblock::GemmIdentityThreadblockSwizzle<>,
      2,
      // >>> Relax global-memory alignment so odd K/N are allowed <<<
      1,   // AlignmentA (elements)
      1,   // AlignmentB
      1    // AlignmentC/D
  >;
  ...
  // Optional: you can check if the launch is supported (alignment, etc.)
  Gemm gemm_op;
  ...
  const double gflops_min = flops_logical / (max_ms * 1e-3) / 1e9;
  const double gflops_max = flops_logical / (min_ms * 1e-3) / 1e9;
  const double gflops_avg = flops_logical / (avg_ms * 1e-3) / 1e9;

  std::cout << "Timing (CUDA events): min " << min_ms << " ms, max " << max_ms << "ms, avg " << avg_ms << " ms\n";
  std::cout << "GFLOPs (logical MxNxK): min " << gflops_min << ", max " << gflops_max << ", avg " << gflops_avg << "\n";

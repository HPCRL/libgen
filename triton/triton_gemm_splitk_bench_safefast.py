...
# ============================================================
# Why you hit the assertion
# ============================================================
# tl.dot() (the MMA/tensorcore path) requires both non-batch dims >= 16.
# Your sweep includes BN=8 (e.g., 32x8x*), which makes b have shape [BK, 8]
# and tl.dot(a[BM,BK], b[BK,BN]) fails when BN < 16.
#
# FIX: Keep your sweep as-is, but inside the kernel:
#   - use tl.dot for BN>=16 (fast path)
#   - use a safe fallback (mul+sum) for BN<16
#
# This preserves correctness and lets you sweep BN=8 configs too.
# ============================================================
...
STAGES_LIST = [3, 4, 5, 6]
SPLITK_LIST = [1, 2, 4, 8, 16]   # sweep split-k
WARPS_LIST  = [1, 2, 4, 8]       # sweep warps
# WARPS_LIST  = [4, 8]       # sweep warps
...
def build_autotune_configs():
    ...
    for cta_str, stages, split_k, warps in itertools.product(
        ...
        # Optional sanity: tl.dot tensorcore path needs BK multiple of 16; your list is fine.
        # We'll keep everything; BN<16 is handled by the fallback inside the kernel.
...
# ============================================================
# Kernel (masked, shape-generic M/N/K)
# - Uses tl.dot for BN>=16 (fast MMA path)
# - Uses safe fallback for BN<16 (mul+sum) so BN=8 configs compile
# ============================================================

@triton.autotune(
    configs=MATMUL_CONFIGS,
    key=["M", "N", "K"],
    reset_to_zero=["C_ptr"],  # critical: atomic_add output during autotune
)
@triton.jit
def gemm_splitk_kernel(
    ...
    while k < k1:
        ...
        k_mask = offs_k < k1  # critical: stay within this split
        ...
        # Fast path: BN >= 16 => tl.dot is allowed
        if BN >= 16:
            acc += tl.dot(a, b)
        else:
            # Fallback path for BN < 16 (e.g., BN=8):
            # (BM,BK) x (BK,BN) -> (BM,BN) via mul+sum
            acc += tl.sum(a.to(tl.float32)[:, :, None] * b.to(tl.float32)[None, :, :], axis=1)
...
def benchmark_one(M, N, K, dtype=torch.float16, device="cuda",
    ...
    # Triton output as fp32 for robust accumulation
    c_triton = torch.empty((M, N), device=device, dtype=torch.float32)
    # cuBLAS output same dtype as inputs
    ...
    # Correctness vs fp32 reference (outside timing)
    ref = a.float() @ b.float()
...
if __name__ == "__main__":
    ...
    # WARNING: This space is huge:
    # 150 CTAs * 4 stages * 5 split-k * 4 warps = 12,000 configs per (M,N,K) key.
    ...
    out_csv = "onepzfull_bench_triton_cta_stages_splitk_warps_vs_cublas.csv"
